...
export class TipoRazaService {
    URL_BACKEND: string = `${environment.url_api}/tiporaza`;

constructor(private http: HttpClient) { }

getAll() {
    return this.http.get<TipoRaza[]>(`${this.URL_BACKEND}`)
    .pipe(catchError(this.handleErrorDefault));
 }

getById(id: number) {
    return this.http.get<TipoRaza>(`${this.URL_BACKEND}/${id}`)
    .pipe(catchError(this.handleErrorDefault));
