...
export class TipoInseminacionService {
    URL_BACKEND: string = `${environment.url_api}/tipoinseminacion`;

constructor(private http: HttpClient) { }

getAll() {
    return this.http.get<TipoInseminacion[]>(`${this.URL_BACKEND}`)
    .pipe(catchError(this.handleErrorDefault));
 }

getById(id: number) {
    return this.http.get<TipoInseminacion>(`${this.URL_BACKEND}/${id}`)
    .pipe(catchError(this.handleErrorDefault));

}

save(req: TipoInseminacion) {
